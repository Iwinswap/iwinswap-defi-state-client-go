...
var _ patcher.TokenSubsystemPatcher = Patcher
...
func Patcher(prevState []tokensystem.TokenView, diff types.TokenSubsystemDiff) ([]tokensystem.TokenView, error) {
	// 1. Create a map from the previous state for efficient manipulation.
	// Since tokensystem.TokenView contains no pointer fields, a direct copy is safe.
	newStateMap := make(map[uint64]tokensystem.TokenView, len(prevState))
	...
	finalState := make([]tokensystem.TokenView, 0, len(newStateMap))
