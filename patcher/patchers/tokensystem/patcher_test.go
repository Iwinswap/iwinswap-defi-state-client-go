...
func findTokenByID(tokens []tokensystem.TokenView, id uint64) *tokensystem.TokenView {
	for i := range tokens {
		if tokens[i].ID == id {
			return &tokens[i]
		}
...
// Helper function to create a new TokenView for testing.
func newTestToken(id uint64, symbol string, fee float64, gas uint64) tokensystem.TokenView {
	return tokensystem.TokenView{
		ID:                   id,
		Symbol:               symbol,
...
func TestPatcher(t *testing.T) {
	...
	initialState := []tokensystem.TokenView{token1Old, token2Old, token3Old}

	t.Run("should handle only additions", func(t *testing.T) {
		token4New := newTestToken(4, "WBTC", 0.0, 50000)
		diff := types.TokenSubsystemDiff{
			Additions: []tokensystem.TokenView{token4New},
		}
	...
	t.Run("should handle only updates", func(t *testing.T) {
		...
		diff := types.TokenSubsystemDiff{
			Updates: []tokensystem.TokenView{token1Updated},
		}
	...
	t.Run("should handle a mix of operations", func(t *testing.T) {
		...
		diff := types.TokenSubsystemDiff{
			Additions: []tokensystem.TokenView{token4New},
			Updates:   []tokensystem.TokenView{token2Updated},
			Deletions: []uint64{3},
		}
