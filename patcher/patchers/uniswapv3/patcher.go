...
var _ patcher.UniswapV3SubsystemPatcher = Patcher
...
func Patcher(prevState []uniswapv3.PoolView, diff types.UniswapV3SubsystemDiff) ([]uniswapv3.PoolView, error) {
	// 1. Create a map from the previous state for efficient manipulation, ensuring a deep copy.
	newStateMap := make(map[uint64]uniswapv3.PoolView, len(prevState))
	...
	finalState := make([]uniswapv3.PoolView, 0, len(newStateMap))
