...
func findPoolByID(pools []uniswapv3.PoolView, id uint64) *uniswapv3.PoolView {
	for i := range pools {
		if pools[i].ID == id {
			return &pools[i]
		}
...
// Helper to create a new PoolView with all relevant fields for testing.
// It performs a deep copy of the input ticks to prevent test data aliasing.
func newTestPool(id uint64, liquidity, sqrtPrice int64, tick int64, tickInfos []uniswapv3.TickInfo) uniswapv3.PoolView {
	var ticksCopy []uniswapv3.TickInfo
	if tickInfos != nil {
		ticksCopy = make([]uniswapv3.TickInfo, len(tickInfos))
		for i, ti := range tickInfos {
			ticksCopy[i] = uniswapv3.TickInfo{
				Index:          ti.Index,
				LiquidityNet:   new(big.Int).Set(ti.LiquidityNet),
				LiquidityGross: new(big.Int).Set(ti.LiquidityGross),
			}
		}
	}

	return uniswapv3.PoolView{
...
func TestPatcher(t *testing.T) {
	// --- Base Data for Tests ---
	tick1 := uniswapv3.TickInfo{Index: 10, LiquidityNet: big.NewInt(100), LiquidityGross: big.NewInt(100)}
	tick2 := uniswapv3.TickInfo{Index: 20, LiquidityNet: big.NewInt(200), LiquidityGross: big.NewInt(200)}

	pool1Old := newTestPool(1, 1000, 5000, 100, []uniswapv3.TickInfo{tick1})
	pool2Old := newTestPool(2, 2000, 6000, 200, []uniswapv3.TickInfo{tick2})
	pool3Old := newTestPool(3, 3000, 7000, 300, nil)

	initialState := []uniswapv3.PoolView{pool1Old, pool2Old, pool3Old}

	t.Run("should handle only additions", func(t *testing.T) {
		pool4New := newTestPool(4, 4000, 8000, 400, nil)
		diff := types.UniswapV3SubsystemDiff{
			Additions: []uniswapv3.PoolView{pool4New},
		}
	...
	t.Run("should handle only updates", func(t *testing.T) {
		pool1Updated := newTestPool(1, 1001, 5005, 101, []uniswapv3.TickInfo{tick1}) // All core fields changed
		diff := types.UniswapV3SubsystemDiff{
			Updates: []uniswapv3.PoolView{pool1Updated},
		}
	...
	t.Run("should verify deep copy on update", func(t *testing.T) {
		...
		localInitialState := []uniswapv3.PoolView{newTestPool(1, 1000, 5000, 100, []uniswapv3.TickInfo{tick1})}

		pool1Updated := newTestPool(1, 1001, 5005, 101, []uniswapv3.TickInfo{tick1})
		diff := types.UniswapV3SubsystemDiff{
			Updates: []uniswapv3.PoolView{pool1Updated},
		}
	...
	t.Run("should handle a mix of operations", func(t *testing.T) {
		pool4New := newTestPool(4, 4000, 8000, 400, nil)
		pool2Updated := newTestPool(2, 2002, 6006, 202, []uniswapv3.TickInfo{tick2})
		diff := types.UniswapV3SubsystemDiff{
			Additions: []uniswapv3.PoolView{pool4New},
			Updates:   []uniswapv3.PoolView{pool2Updated},
			Deletions: []uint64{3},
		}
