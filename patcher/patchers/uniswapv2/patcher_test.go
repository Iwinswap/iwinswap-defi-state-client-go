...
func findPoolByID(pools []uniswapv2.PoolView, id uint64) *uniswapv2.PoolView {
	for i := range pools {
		if pools[i].ID == id {
			return &pools[i]
		}
...
func TestPatcher(t *testing.T) {
	// --- Base Data for Tests ---
	pool1Old := uniswapv2.PoolView{ID: 1, Reserve0: big.NewInt(1000), Reserve1: big.NewInt(5000)}
	pool2Old := uniswapv2.PoolView{ID: 2, Reserve0: big.NewInt(2000), Reserve1: big.NewInt(6000)}
	pool3Old := uniswapv2.PoolView{ID: 3, Reserve0: big.NewInt(3000), Reserve1: big.NewInt(7000)}

	initialState := []uniswapv2.PoolView{pool1Old, pool2Old, pool3Old}

	t.Run("should handle only additions", func(t *testing.T) {
		pool4New := uniswapv2.PoolView{ID: 4, Reserve0: big.NewInt(4000)}
		diff := types.UniswapV2SubsystemDiff{
			Additions: []uniswapv2.PoolView{pool4New},
		}
	...
	t.Run("should handle only updates", func(t *testing.T) {
		pool1Updated := uniswapv2.PoolView{ID: 1, Reserve0: big.NewInt(1001), Reserve1: big.NewInt(5005)} // Reserves changed
		diff := types.UniswapV2SubsystemDiff{
			Updates: []uniswapv2.PoolView{pool1Updated},
		}
	...
	t.Run("should verify deep copy on update", func(t *testing.T) {
		...
		localInitialState := []uniswapv2.PoolView{
			{ID: 1, Reserve0: big.NewInt(1000), Reserve1: big.NewInt(5000)},
		}

		pool1Updated := uniswapv2.PoolView{ID: 1, Reserve0: big.NewInt(1001), Reserve1: big.NewInt(5005)}
		diff := types.UniswapV2SubsystemDiff{
			Updates: []uniswapv2.PoolView{pool1Updated},
		}
	...
	t.Run("should handle a mix of operations", func(t *testing.T) {
		...
		pool4New := uniswapv2.PoolView{ID: 4, Reserve0: big.NewInt(4000)}
		pool2Updated := uniswapv2.PoolView{ID: 2, Reserve0: big.NewInt(2002)}
		diff := types.UniswapV2SubsystemDiff{
			Additions: []uniswapv2.PoolView{pool4New},
			Updates:   []uniswapv2.PoolView{pool2Updated},
			Deletions: []uint64{3},
		}
