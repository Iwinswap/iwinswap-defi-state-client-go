...
var _ patcher.TokenPoolSubsystemPatcher = Patcher

// deepCopyView creates a new TokenPoolsRegistryView with its own memory for all its slices.
// This is essential to prevent the new state from sharing memory with the diff object.
func deepCopyView(v *poolregistry.TokenPoolsRegistryView) *poolregistry.TokenPoolsRegistryView {
	if v == nil {
		return nil
	}
	newV := &poolregistry.TokenPoolsRegistryView{}
...
// Patcher is a concrete implementation of the TokenPoolSubsystemPatcher function type.
...
func Patcher(prevState *poolregistry.TokenPoolsRegistryView, diff types.TokenPoolSubsystemDiff) (*poolregistry.TokenPoolsRegistryView, error) {
