...
func TestPatcher(t *testing.T) {
	// --- Base Data for Tests ---
	oldState := &poolregistry.TokenPoolsRegistryView{
		Tokens: []uint64{1, 2},
		Pools:  []uint64{10},
	}

	newStateData := &poolregistry.TokenPoolsRegistryView{
	...
	t.Run("should correctly replace the old state with the new state", func(t *testing.T) {
		diff := types.TokenPoolSubsystemDiff{
			Data: newStateData,
		}

		newState, err := Patcher(oldState, diff)
