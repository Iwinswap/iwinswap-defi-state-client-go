...
import (
	...
	"github.com/defistate/defi-state-client-go/graph/bitset"
	...
	uniswapv2calculator "github.com/defistate/defi-state-client-go/calculators/uniswapv2"
	uniswapv3calculator "github.com/defistate/defi-state-client-go/calculators/uniswapv3"
)
...
type Graph struct {
	...
	tokenPool *poolregistry.TokenPoolsRegistryView
...
func NewGraph(
	tokenPool *poolregistry.TokenPoolsRegistryView,
	tokenRegistryView []token.TokenView,
	poolRegistryView poolregistry.PoolRegistryView,
	...
	indexedPoolRegistry := poolregistry.NewIndexablePoolRegistry(poolRegistryView.Pools)
	indexedTokenRegistry := token.NewIndexableTokenSystem(tokenRegistryView)
	...
	for i, poolID := range tokenPool.Pools {
		...
		case uniswapv2.UniswapV2ProtocolSchema:
			...
			if !ok {
				...
				indexedProtocol = uniswapv2.NewIndexableUniswapV2System(protocol.Data.([]uniswapv2.PoolView))
				protocolIDToIndexed[protocolId] = indexedProtocol
			}

			// get pool from protocol
			pool, found := indexedProtocol.(*uniswapv2.IndexableUniswapV2System).GetByID(poolID)
			if !found {
		...
		case uniswapv3.UniswapV3ProtocolSchema:
			...
			if !ok {
				// create the indexed protocol
				indexedProtocol = uniswapv3.NewIndexableUniswapV3System(protocol.Data.([]uniswapv3.PoolView))
				protocolIDToIndexed[protocolId] = indexedProtocol
			}

			// get pool from protocol
			pool, found := indexedProtocol.(*uniswapv3.IndexableUniswapV3System).GetByID(poolID)
			if !found {
				continue // maybe panic?
			}
...
func (g *Graph) FindBestSwapPath(
	...
	if !exists {
		return nil, nil, fmt.Errorf("start token %d not found in the graph", tokenInID)
	}

	endIndex, exists := g.tokenToIndex[tokenOutID]
	if !exists {
		return nil, nil, fmt.Errorf("end token %d not found in the graph", tokenOutID)
	}
